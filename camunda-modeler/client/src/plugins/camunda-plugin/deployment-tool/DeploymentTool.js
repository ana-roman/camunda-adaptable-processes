/* eslint-disable no-trailing-spaces,lines-around-comment,no-unused-vars */
/**
...
export default class DeploymentTool extends PureComponent {
  ...
  deploy = (options = {}) => {
    ...
    this.state.deploymentMode = 'deployment';
    return this.deployTab(activeTab, options);
  }

  deployAdaptable = (options = {}) => {
    const {
      activeTab
    } = this.state;

    this.state.deploymentMode = 'adaptable';
    return this.deployTab(activeTab, options);
  }
  ...
  render() {
    ...
    return <React.Fragment>
      { activeTab && activeTab.type !== 'empty' && <Fill slot="toolbar" group="8_deploy">
        ...
        <Button
          onClick={ this.deployAdaptable }
          title="Deploy Adaptable process"
        >
          <Icon name="deploy" />
        </Button>
      </Fill> }
